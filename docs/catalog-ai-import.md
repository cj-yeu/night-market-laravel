...
**Current status:** the 2026-09-16 Market-import fix below is local and uncommitted. Real Tavily article search and article/video-segment extraction succeeded with 3.5 Flash-Lite; isolated Laravel workflow tests pass. New-code logged-in browser acceptance, human confirmation of extracted video details/photos and production private-image persistence remain pending. Historical diagnostics below describe earlier work, not deployment verification of this fix.

## Market import and article search fix — 2026-09-16

- The supplied patch was reviewed and adapted, not blindly accepted. Module imports can create a new **inactive** Market using the Admin-entered name/city when a video only supplies Stall/Food evidence. Unknown address/schedule remain empty. Checked schedule entries must still contain valid times. Legacy metadata proposals retain their stricter source-evidence, address and schedule requirements.
- The new Market selection is retained through analysis and visible in Draft/Review Import. Later sources can enrich missing Market details after Stalls have already been extracted. They cannot overwrite edited fields, restore an Admin-cleared schedule, or attach explicit evidence for another city/state. Market evidence links remain available for review. Existing catalog records are not overwritten; duplicate Market detection also covers incomplete addresses.
- Search queries now focus on market name/city/state rather than demanding all catalog fields. Articles exclude social-video sites as well as YouTube. A real overlong query returned HTTP 200 with zero results; the concise query returned eight sources, and excluding social-video sites yielded readable article/guide links including KL Foodie. This establishes a local query-quality issue, not the exact cause of any uninspected Railway error.
- Failed/empty searches are not reused by the five-minute success cache. Missing key/free-plan confirmation, HTTP authorization/quota errors and DNS/TLS/timeout failures have distinct safe messages. No arbitrary provider error body or credential is displayed. GET result refresh remains free of provider calls.
- The structured schema now permits unnamed Stall drafts, matching the existing prompt and validator. Their parent ownership/name still require Admin confirmation; this does not invent brands, prices, Halal evidence or photos.

### Verification for this fix

- Local runtime configuration: Gemini/Tavily/YouTube keys present; Catalog model `gemini-3.5-flash-lite`; both free-plan confirmation flags true; no configuration cache. The flags are operator attestation, not provider billing/quota verification. No `.env` edit or Railway inspection/configuration change was made.
- Tavily: four application attempts total (one sandbox transport failure without an HTTP response; three HTTP 200 responses, **3 reported credits**). Final article query returned eight public sources. No automatic retry or paid fallback.
- Article: the retrieved KL Foodie guide followed a 301 → 200 redirect and provided 6,320 characters of actual body text. Gemini structured extraction returned HTTP 200: SS2 Market context, two unnamed Stall drafts and two source-supported Food prices (RM1 and RM3.70). Unknown address/schedule remained empty. Eight page image candidates are **not** eight verified Food photos. The first in-memory harness stopped before Gemini because a relation loader expected a configured connection; it was corrected without enabling any database connection, and the public body was fetched again.
- Video: the previously verified public SS2 video `nGAsX56rGvI`, seconds 0–120 only, returned HTTP 200 with VIDEO input usage. Its structured extraction also returned HTTP 200: nine Stall drafts and eight Foods, including unknown names/prices. This is segment analysis, not full-video analysis or independent human verification. It produced **zero image files**; no thumbnail was assigned as a Food photo.
- Gemini: **3 actual generateContent calls**, all HTTP 200; returned totals **14,931 input + 4,202 output = 19,133 tokens**. No model-list, Gemini Search grounding, OpenAI or Calendar request. All live checks ran with database connections disabled and did not create drafts/catalog records.
- Automated Laravel HTTP workflow/regression checks: **78 passed / 651 assertions** across `CatalogAiImportTest`, `CatalogSourceSearchTest`, `SocialMediaGeminiSuggestionTest`, and `SocialMediaCatalogImportTest`. Gate confirmed `testing / mysql / night_market_laravel_testing / 127.0.0.1:3306` and actual `SELECT DATABASE()`. DatabaseTransactions rolled back fixtures; external calls used Http fakes. Coverage includes new Market → Stall → Food, selected images, parent links, incomplete drafts, cross-city rejection, source enrichment, unchanged legacy checks, and idempotent repeated import.
- Explicit-file Pint, PHP syntax and JavaScript syntax checks, plus `git diff --check`, are used for this fix. No migration/schema rebuild, shared-data deletion, full-suite run, commit/push or deployment.
- Remaining acceptance: deploy only after review; on the deployed fix an Admin should check Search → Select → Analyse → Save Draft → Review Import at desktop/mobile sizes, then explicitly authorize any real catalog import. Automated route/render tests and direct provider calls do **not** constitute logged-in browser acceptance. Private draft persistence remains the separate operator task below.
...
- Source variants retain their own evidence/prices; no averaging or automatic overwrite. Existing Stall/Food links are revalidated under locks. New records are inactive; Halal remains Unknown. New Market-only imports can be explicitly selected with a name/city; unknown address/schedule remain incomplete for later Admin verification.
