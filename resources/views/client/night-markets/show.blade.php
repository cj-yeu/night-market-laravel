            ...
            <section class="card market-card mb-4" aria-labelledby="market-reviews-heading">
                <div class="card-body p-4 p-md-5">
                    ...
                    @else
                        ...
                        <div class="vstack gap-3">@foreach ($reviews as $review)<article class="border rounded-3 p-3 bg-white"><div class="d-flex justify-content-between gap-3 mb-2"><div class="d-flex align-items-center gap-2"><x-user-avatar :user="$review->user" size="sm" /><strong>{{ $review->user->name }}</strong></div><span class="badge text-bg-warning" aria-label="{{ $review->rating }} out of 5 stars">{{ $review->rating }}/5</span></div><p class="mb-1">{{ $review->comment }}</p>@if ($review->tags->isNotEmpty())<div class="d-flex flex-wrap gap-1 mb-2">@foreach ($review->tags as $tag)<span class="badge text-bg-light border text-dark">{{ $tag->name }}</span>@endforeach</div>@endif<small class="text-secondary">{{ $review->created_at->format('M j, Y') }}@if ($review->updated_at->gt($review->created_at)) · Updated {{ $review->updated_at->format('M j, Y') }}@endif</small></article>@endforeach</div>
