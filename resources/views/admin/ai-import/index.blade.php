...
<p class="small mb-1">This Laravel process: TAVILY_API_KEY {{ $searchStatus['article_key_present'] ? 'present' : 'missing' }} · Free-plan confirmation {{ $searchStatus['article_free_confirmed'] ? 'on' : 'off' }}. Presence does not prove the key is accepted or has quota.</p>
...
<fieldset class="col-12"><legend class="h5">What would you like to import?</legend><div class="ai-mode-grid">
@foreach(['new_market'=>'Create New Night Market with Stalls and Foods','existing_market'=>'Add Stalls and Foods to Existing Night Market'] as $mode => $label)
<label class="ai-mode-card"><input type="radio" class="form-check-input" name="import_mode" value="{{ $mode }}" required @checked(old('import_mode',$context['import_mode'] ?? null) === $mode)><span>{{ $label }}</span></label>
@endforeach</div><p class="small mt-2">Choose explicitly. New records remain inactive until reviewed and activated.</p></fieldset>
...
<form action="{{ route('admin.ai-import.prepare') }}" method="POST" data-ai-busy="Reading selected sources and preparing your import…">@csrf<input type="hidden" name="search_id" value="{{ $searchId }}">
...
<a href="{{ $source['url'] }}" target="_blank" rel="noopener noreferrer">Open Source <span class="visually-hidden">(new tab)</span></a><label class="ai-check mt-auto"><input class="form-check-input" type="checkbox" name="source_ids[]" value="{{ $i }}" @checked(in_array($i,old('source_ids',count($result['sources']) === 1 ? [0] : [])))><span data-source-selection>{{ count($result['sources']) === 1 ? 'Selected' : 'Not selected' }}</span><span class="visually-hidden">: {{ $source['title'] }}</span></label></div></article>
...
@include('admin.ai-import._video-range', ['rangeId'=>'search'])
<div class="ai-selection-bar"><span data-selection-count>{{ count($result['sources']) === 1 ? 1 : 0 }} selected sources</span><button class="btn btn-market" type="submit" @disabled(!$result['sources'])>Analyse &amp; Prepare Import</button><span role="status" data-busy-status></span></div><p class="small mt-2">One video segment or up to three articles at a time. Your work is saved automatically; you will review and confirm before any catalog record is created.</p></form>
...
<details class="card market-card mt-4" @if(! $searchStatus['available']) open @endif><summary class="p-3 fw-bold">Have a source already? Paste URL</summary><div class="card-body"><p>Choose the import mode, name and city above first. Public articles and YouTube videos are supported.</p><form action="{{ route('admin.ai-import.prepare') }}" method="POST" enctype="multipart/form-data" data-paste-source data-ai-busy="Reading source and preparing your import…">@csrf
@foreach(['import_mode','module','market_id','stall_id','name','city'] as $key)<input type="hidden" name="{{ $key }}" value="{{ $context[$key] ?? '' }}">@endforeach
<label for="source-url" class="form-label">Source URL</label><input type="url" id="source-url" name="url" class="form-control mb-3" placeholder="https://…" required>
@include('admin.ai-import._video-range', ['rangeId'=>'paste'])
<details class="mb-3"><summary>Supply source text or a screenshot instead</summary><label class="form-label w-100">Text from this source<textarea class="form-control" name="text" rows="4" maxlength="30000"></textarea></label><label class="form-label w-100">Source screenshot<input class="form-control" name="screenshot" type="file" accept="image/jpeg,image/png,image/webp"></label><p class="small">This will be labelled as Admin-provided evidence, not video content analysis.</p></details>
<button class="btn btn-market">Analyse &amp; Prepare Import</button><span role="status" data-busy-status></span></form></div></details>
</div>
@endsection
