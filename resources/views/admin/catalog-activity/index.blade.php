...
@section('content')
    <div class="mx-auto" style="max-width: 1200px;">
        ...
        <form method="GET" class="card market-card mb-4"><div class="card-body p-3"><div class="row g-3 align-items-end">
            ...
            <div class="col-6 col-md-3"><label for="entity_type" class="form-label">Entity type</label><select id="entity_type" name="entity_type" class="form-select"><option value="">All types</option><option value="night_market" @selected(($filters['entity_type'] ?? '') === 'night_market')>Night Market</option><option value="stall" @selected(($filters['entity_type'] ?? '') === 'stall')>Stall</option><option value="food" @selected(($filters['entity_type'] ?? '') === 'food')>Food</option><option value="user" @selected(($filters['entity_type'] ?? '') === 'user')>User</option></select></div>
            <div class="col-6 col-md-3"><label for="action" class="form-label">Action</label><select id="action" name="action" class="form-select"><option value="">All actions</option>@foreach (['created' => 'Created', 'updated' => 'Updated', 'activated' => 'Activated', 'deactivated' => 'Deactivated', 'image_updated' => 'Image updated', 'image_removed' => 'Image removed', 'role_promoted' => 'Promoted Client to Admin', 'role_demoted' => 'Demoted Admin to Client'] as $value => $label)<option value="{{ $value }}" @selected(($filters['action'] ?? '') === $value)>{{ $label }}</option>@endforeach</select></div>
            <div class="col-12 d-flex gap-2"><button class="btn btn-market">Apply Filters</button><a class="btn btn-outline-secondary" href="{{ route('admin.catalog-activity.index') }}">Reset Filters</a></div>
        </div></div></form>
        ...
        <div class="card market-card"><div class="card-body p-0"><div class="table-responsive"><table class="table align-middle mb-0">
            ...
            @empty<tr><td colspan="6" class="text-center text-secondary py-5">No catalog activity matches these filters. <a href="{{ route('admin.catalog-activity.index') }}">Reset Filters</a></td></tr>@endforelse</tbody>
        </table></div></div></div>
