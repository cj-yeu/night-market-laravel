        ...
        <div class="card market-card"><div class="card-body p-4 p-md-5">
            <form method="POST" action="{{ route('admin.night-markets.update', $nightMarket) }}" novalidate>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6"><label for="city" class="form-label">City</label><select id="city" name="city" class="form-select @error('city') is-invalid @enderror" required><option value="">Select a Selangor city or town</option>@foreach ($cities as $city)<option value="{{ $city }}" @selected(old('city', $nightMarket->city) === $city)>{{ $city }}</option>@endforeach</select><div class="form-text">Existing locations remain selectable for backward compatibility.</div>@error('city')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
